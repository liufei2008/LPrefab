...
namespace LPrefabSystem4
{
	...
	AActor* ActorSerializer::DeserializeActor(USceneComponent* Parent, ULPrefab* InPrefab, bool ReplaceTransform, FVector InLocation, FQuat InRotation, FVector InScale)
	{
		if (!InPrefab)
		{
		...
		if (!TargetWorld)
		{
			UE_LOG(LPrefab, Error, TEXT("Load Prefab, World is null!"));
			return nullptr;
		}
		...
		FLPrefabSaveData SaveData;
		{
			...
			if (LoadedData.Num() <= 0)
			{
				UE_LOG(LPrefab, Warning, TEXT("Loaded data is empty!"));
				return nullptr;
			}
		...
		UE_LOG(LPrefab, Log, TEXT("Take %fs loading prefab: %s"), TimeSpan.GetTotalSeconds(), *InPrefab->GetName());
		...
		for (auto& ObjectData : SavedComponents)
		{
			...
			else
			{
				if (auto ObjectClass = FindClassFromListByIndex(ObjectData.ObjectClass))
				{
					...
					{
						UE_LOG(LPrefab, Warning, TEXT("[ActorSerializer::PreGenerateObjectArray]Wrong component class: %s"), *(ObjectClass->GetFName().ToString()));
						continue;
					}
					...
					{
						UE_LOG(LPrefab, Warning, TEXT("[ActorSerializer::PreGenerateObjectArray]Missing Owner actor when creating component: %s"), *(ObjectData.ComponentName.ToString()));
						continue;
					}
				}
			}
		...
		for (auto& ObjectData : SavedObjects)
		{
			...
			else
			{
				if (auto ObjectClass = FindClassFromListByIndex(ObjectData.ObjectClass))
				{
					...
					{
						UE_LOG(LPrefab, Warning, TEXT("[ActorSerializer::PreGenerateObjectArray]Wrong object class: %s"), *(ObjectClass->GetFName().ToString()));
						continue;
					}
					...
					{
						UE_LOG(LPrefab, Warning, TEXT("[ActorSerializer::PreGenerateObjectArray]Missing Outer object when creating object of type: %s"), *(ObjectClass->GetFName().ToString()));
						continue;
					}
				}
			}
	...
	void ActorSerializer::PreGenerateActorRecursive(FLGUIActorSaveData& InActorData, USceneComponent* Parent)
	{
		...
		else
		{
			if (auto ActorClass = FindClassFromListByIndex(InActorData.ObjectClass))
			{
				...
				{
					UE_LOG(LPrefab, Warning, TEXT("[ActorSerializer::PreGenerateActorRecursive]Find class: '%s' at index: %d, but is not a Actor class, use default."), *(ActorClass->GetFName().ToString()), InActorData.ObjectClass);
			...
			{
				UE_LOG(LPrefab, Warning, TEXT("[ActorSerializer::PreGenerateActorRecursive]Actor Class of index:%d not found!"), (InActorData.ObjectClass));
			}
		}
	}
