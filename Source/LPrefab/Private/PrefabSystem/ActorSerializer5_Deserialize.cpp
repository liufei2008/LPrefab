...
#endif
namespace LPrefabSystem5
{
	AActor* ActorSerializer::LoadPrefabWithExistingObjects(UWorld* InWorld, ULPrefab* InPrefab, USceneComponent* Parent
		, TMap<FGuid, TObjectPtr<UObject>>& InOutMapGuidToObjects, TMap<TObjectPtr<AActor>, FLSubPrefabData>& OutSubPrefabMap
	)
	{
		...
		serializer.WriterOrReaderFunction = [&serializer](UObject* InObject, TArray<uint8>& InOutBuffer, bool InIsSceneComponent) {
			...
			LPrefabSystem::FLPrefabObjectReader Reader(InOutBuffer, serializer, ExcludeProperties);
		...
		serializer.WriterOrReaderFunctionForSubPrefab = [&serializer](UObject* InObject, TArray<uint8>& InOutBuffer, const TArray<FName>& InOverridePropertyNames) {
			LPrefabSystem::FLPrefabOverrideParameterObjectReader Reader(InOutBuffer, serializer, InOverridePropertyNames);
			Reader.DoSerialize(InObject);
		};
	...
	AActor* ActorSerializer::LoadPrefab(UWorld* InWorld, ULPrefab* InPrefab, USceneComponent* Parent, bool SetRelativeTransformToIdentity, TFunction<void(AActor*)> CallbackBeforeAwake)
	{
		ActorSerializer serializer;
		serializer.TargetWorld = InWorld;
	...
	AActor* ActorSerializer::LoadPrefab(UWorld* InWorld, ULPrefab* InPrefab, USceneComponent* Parent, FVector RelativeLocation, FQuat RelativeRotation, FVector RelativeScale, TFunction<void(AActor*)> CallbackBeforeAwake)
	{
		ActorSerializer serializer;
		serializer.TargetWorld = InWorld;
		serializer.CallbackBeforeAwake = CallbackBeforeAwake;
	...
	AActor* ActorSerializer::LoadPrefab(UWorld* InWorld, ULPrefab* InPrefab, USceneComponent* Parent, const TMap<UObject*, UObject*>& InReplaceAssetMap, const TMap<UClass*, UClass*>& InReplaceClassMap, TFunction<void(AActor*)> CallbackBeforeAwake)
	{
		ActorSerializer serializer;
		serializer.TargetWorld = InWorld;
		serializer.CallbackBeforeAwake = CallbackBeforeAwake;
	...
	AActor* ActorSerializer::LoadSubPrefab(
		UWorld* InWorld, ULPrefab* InPrefab, USceneComponent* Parent
...
#define LGUIPREFAB_LOG_DETAIL_TIME 0
	AActor* ActorSerializer::DeserializeActorFromData(FLPrefabSaveData& SaveData, USceneComponent* Parent, bool ReplaceTransform, FVector InLocation, FQuat InRotation, FVector InScale)
	{
		...
		{
			LPrefabManager = ULPrefabWorldSubsystem::GetInstance(TargetWorld);
		}
		if (!bIsSubPrefab)
		{
			if (!DeserializationSessionId.IsValid())
			{
				...
				LPrefabManager->BeginPrefabSystemProcessingActor(DeserializationSessionId);
			}
		}
...
#if LGUIPREFAB_LOG_DETAIL_TIME
		UE_LOG(LPrefab, Log, TEXT("--DeserializeObject take time: %fms"), (FDateTime::Now() - Time).GetTotalMilliseconds());
		Time = FDateTime::Now();
#endif
		...
		if (!bIsSubPrefab)
		{
			...
			{
				LPrefabManager->RemoveActorForPrefabSystem(item, DeserializationSessionId);
			}
			LPrefabManager->EndPrefabSystemProcessingActor(DeserializationSessionId);
		}
#if LGUIPREFAB_LOG_DETAIL_TIME
		UE_LOG(LPrefab, Log, TEXT("--Call Awake (and OnEnable) take time: %fms"), (FDateTime::Now() - Time).GetTotalMilliseconds());
#endif

		return CreatedRootActor;
	}
	AActor* ActorSerializer::DeserializeActor(USceneComponent* Parent, ULPrefab* InPrefab, const TFunction<void()>& InCallbackBeforeDeserialize, bool ReplaceTransform, FVector InLocation, FQuat InRotation, FVector InScale)
	{
		if (!InPrefab)
		{
			UE_LOG(LPrefab, Error, TEXT("Load Prefab, InPrefab is null!"));
			return nullptr;
		}
		if (!TargetWorld)
		{
			UE_LOG(LPrefab, Error, TEXT("Load Prefab: '%s', World is null!"), *InPrefab->GetPathName());
			return nullptr;
		}

#if LGUIPREFAB_LOG_DETAIL_TIME
		UE_LOG(LPrefab, Log, TEXT("Begin load prefab: '%s'"), *InPrefab->GetName());
#endif
		...
		if (ULPrefabSettings::GetLogPrefabLoadTime())
		{
			...
			UE_LOG(LPrefab, Log, TEXT("End load prefab: '%s', total time: %fms"), *InPrefab->GetName(), TimeSpan.GetTotalMilliseconds());
		}
	...
	void ActorSerializer::PreGenerateObjectArray(const TArray<FLGUIObjectSaveData>& SavedObjects, const TArray<FLGUIComponentSaveData>& SavedComponents)
	{
		...
		for (auto& ObjectData : SavedComponents)
		{
			...
			{
				//UE_LOG(LPrefab, Log, TEXT("[ActorSerializer::PreGenerateObjectArray]Already generated:%s!"), *(MapGuidToObject[ObjectData.ComponentGuid]->GetPathName()));
			...
			else
			{
				if (auto ObjectClass = FindClassFromListByIndex(ObjectData.ObjectClass))
				{
					...
					{
						UE_LOG(LPrefab, Warning, TEXT("[ActorSerializer::PreGenerateObjectArray]Wrong component class: '%s'. Prefab: '%s'"), *(ObjectClass->GetFName().ToString()), *PrefabAssetPath);
						continue;
					}
					...
					{
						UE_LOG(LPrefab, Warning, TEXT("[ActorSerializer::PreGenerateObjectArray]Missing Owner actor when creating component: %s. Prefab: '%s'"), *(ObjectData.ComponentName.ToString()), *PrefabAssetPath);
						continue;
					}
				}
			}
			...
			if (CreatedNewComponent)
			{
				...
				for (auto DefaultSubObject : DefaultSubObjects)
				{
					...
					if (Index == INDEX_NONE)
					{
						UE_LOG(LPrefab, Warning, TEXT("[ActorSerializer::PreGenerateObjectArray]Missing guid for default sub object: %s"), *(DefaultSubObject->GetFName().ToString()));
						continue;
					}
		...
		for (auto& ObjectData : SavedObjects)
		{
			...
			{
				//UE_LOG(LPrefab, Log, TEXT("[ActorSerializer::PreGenerateObjectArray]Already generated:%s!"), *(MapGuidToObject[ObjectData.ObjectGuid]->GetPathName()));
			...
			else
			{
				if (auto ObjectClass = FindClassFromListByIndex(ObjectData.ObjectClass))
				{
					...
					{
						UE_LOG(LPrefab, Warning, TEXT("[ActorSerializer::PreGenerateObjectArray]Wrong object class: '%s'. Prefab: '%s'"), *(ObjectClass->GetFName().ToString()), *PrefabAssetPath);
						continue;
					}
					...
					{
						UE_LOG(LPrefab, Warning, TEXT("[ActorSerializer::PreGenerateObjectArray]Missing Outer object when creating object: '%s'. Prefab: '%s'"), *(ObjectData.ObjectName.ToString()), *PrefabAssetPath);
						continue;
					}
				}
			}
	...
	void ActorSerializer::PreGenerateActorRecursive(FLGUIActorSaveData& InActorData, USceneComponent* Parent)
	{
		if (InActorData.bIsPrefab)
		{
			...
			{
				if (auto SubPrefabAsset = Cast<ULPrefab>(PrefabAssetObject))
				{
					AActor* SubPrefabRootActor = nullptr;
					FLSubPrefabData SubPrefabData;
					SubPrefabData.PrefabAsset = SubPrefabAsset;

					if (SubPrefabAsset->PrefabVersion <= (uint16)ELPrefabVersion::ObjectName)
					{
						...
						switch ((ELPrefabVersion)SubPrefabAsset->PrefabVersion)
						{
						case ELPrefabVersion::BuildinFArchive:
						{
							SubPrefabRootActor = LPrefabSystem3::ActorSerializer::LoadSubPrefab(this->TargetWorld, SubPrefabAsset, Parent, DeserializationSessionId, this->ActorIndexInPrefab, SubMapGuidToObject
						...
						{
							SubPrefabRootActor = LPrefabSystem4::ActorSerializer::LoadSubPrefab(this->TargetWorld, SubPrefabAsset, Parent, DeserializationSessionId, this->ActorIndexInPrefab, SubMapGuidToObject
								, NewOnSubPrefabFinishDeserializeFunction
							);
						}
						break;
						...
						{
							SubPrefabRootActor = LPrefabSystem5::ActorSerializer::LoadSubPrefab(this->TargetWorld, SubPrefabAsset, Parent, DeserializationSessionId, this->ActorIndexInPrefab, SubMapGuidToObject
								, NewOnSubPrefabFinishDeserializeFunction
							);
						}
						break;
						default:
						{
							...
							LPrefabUtils::EditorNotification(MsgText, 1.0f);
						}
						break;
						}
					}
					else
					{
						...
						case ELPrefabVersion::ActorAttachToSubPrefab:
						{
							...
							if ((ELPrefabVersion)SubPrefabAsset->PrefabVersion == ELPrefabVersion::CommonActor)
							{
								SubPrefabRootActor = LPrefabSystem6::ActorSerializer::LoadSubPrefab(this->TargetWorld, SubPrefabAsset, Parent, DeserializationSessionId, this->ActorIndexInPrefab, SubMapGuidToObject
									, NewOnSubPrefabFinishDeserializeFunction
								);
							}
							else
							{
								SubPrefabRootActor = LPrefabSystem7::ActorSerializer::LoadSubPrefab(this->TargetWorld, SubPrefabAsset, Parent, DeserializationSessionId, this->ActorIndexInPrefab, SubMapGuidToObject
									, NewOnSubPrefabFinishDeserializeFunction
								);
							}
						}
						...
						case ELPrefabVersion::NewObjectOnNestedPrefab:
						{
							...
							SubPrefabRootActor = LPrefabSystem8::ActorSerializer::LoadSubPrefab(this->TargetWorld, SubPrefabAsset, Parent, DeserializationSessionId, SubMapGuidToObject
								, NewOnSubPrefabFinishDeserializeFunction
							);
						}
						break;
		...
		else
		{
			if (auto ActorClass = FindClassFromListByIndex(InActorData.ObjectClass))
			{
				...
				{
					UE_LOG(LPrefab, Warning, TEXT("[ActorSerializer::PreGenerateActorRecursive]Find class: '%s' at index: %d, but is not a Actor class, use default. Prefab: '%s'"), *(ActorClass->GetFName().ToString()), InActorData.ObjectClass, *PrefabAssetPath);
				...
				LPrefabManager->AddActorForPrefabSystem(NewActor, DeserializationSessionId);
				if (bNeedFinishSpawn)
				{
				...
				for (auto DefaultSubObject : DefaultSubObjects)
				{
					...
					if (Index == INDEX_NONE)
					{
						UE_LOG(LPrefab, Warning, TEXT("[ActorSerializer::PreGenerateActorRecursive]Missing guid for default sub object: %s"), *(DefaultSubObject->GetFName().ToString()));
						continue;
					}
			...
			{
				UE_LOG(LPrefab, Warning, TEXT("[ActorSerializer::PreGenerateActorRecursive]Actor Class of index:%d not found! Prefab: '%s'"), (InActorData.ObjectClass), *PrefabAssetPath);
			}
		}
	}
				...
				if (auto SubPrefabDataPtr = SubPrefabMap.Find(NewActor))
				{
					for (auto& RecordData : InActorData.ObjectOverrideParameterArray)
					{
						...
						if (ObjectPtr != nullptr)
						{
							...
							FLPrefabOverrideParameterData OverrideDataItem;
