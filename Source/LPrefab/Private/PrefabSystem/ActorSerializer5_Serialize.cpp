...
namespace LPrefabSystem5
{
	...
	void ActorSerializer::SerializeActor(AActor* OriginRootActor, ULPrefab* InPrefab)
	{
		if (!InPrefab)
		{
			UE_LOG(LPrefab, Error, TEXT("Save Prefab, InPrefab is null!"));
			return;
		}
		if (!IsValid(OriginRootActor))
		{
			UE_LOG(LPrefab, Error, TEXT("Save Prefab, OriginRootActor is not valid!"));
			return;
		}
		if (OriginRootActor->HasAnyFlags(EObjectFlags::RF_Transient))
		{
			UE_LOG(LPrefab, Error, TEXT("Save Prefab, OriginRootActor is transient!"));
			return;
		}
