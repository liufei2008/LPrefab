	...
	void ActorSerializer::PreGenerateActorRecursive(FLGUIActorSaveData& InActorData, USceneComponent* Parent)
	{
		...
		else
		{
			if (auto ActorClass = FindClassFromListByIndex(InActorData.ActorClass))
			{
				...
				{
					UE_LOG(LPrefab, Warning, TEXT("[ActorSerializer::PreGenerateActorRecursive]Find class: '%s' at index: %d, but is not a Actor class, use default."), *(ActorClass->GetFName().ToString()), InActorData.ActorClass);
			...
			{
				UE_LOG(LPrefab, Warning, TEXT("[ActorSerializer::PreGenerateActorRecursive]Actor Class of index:%d not found!"), (InActorData.ActorClass));
			}
		}
	}
