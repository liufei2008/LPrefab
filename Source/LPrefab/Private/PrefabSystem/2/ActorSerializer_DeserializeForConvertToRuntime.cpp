...
void ActorSerializer::DeserializeActorRecursiveForConvertToRuntime(const FLGUIActorSaveData& SaveData, FLGUIActorSaveDataForBuild& ResultSaveData)
{
	...
	if (auto ActorClass = FindClassFromListByIndex(SaveData.ActorClass, Prefab.Get()))
	{
		...
		{
			UE_LOG(LPrefab, Warning, TEXT("[ActorSerializer::DeserializeActorRecursiveForConvertToRuntime]Find class: '%s' at index: %d, but is not a Actor class, use default"), *(ActorClass->GetFName().ToString()), SaveData.ActorClass);
		...
		if (RootCompSaveData.ComponentClass != -1)//have RootComponent data
		{
			if (auto CompClass = FindClassFromListByIndex(RootCompSaveData.ComponentClass, Prefab.Get()))
			{
				...
				{
					UE_LOG(LPrefab, Warning, TEXT("[ActorSerializer::DeserializeActorRecursiveForConvertToRuntime]Find class: '%s' at index: %d, but is not a USceneComponent, use default"), *(CompClass->GetFName().ToString()), RootCompSaveData.ComponentClass);
		...
		for (int i = 1; i < ComponentCount; i++)//start from 1, skip RootComponent
		{
			...
			if (auto CompClass = FindClassFromListByIndex(CompData.ComponentClass, Prefab.Get()))
			{
				...
				{
					UE_LOG(LPrefab, Warning, TEXT("[ActorSerializer::DeserializeActorRecursiveForConvertToRuntime]Find class: '%s' at index: %d, but is not a USceneComponent, use default"), *(CompClass->GetFName().ToString()), CompData.ComponentClass);
			...
			{
				UE_LOG(LPrefab, Warning, TEXT("[ActorSerializer::DeserializeActorRecursiveForConvertToRuntime]Component Class of index:%d not found!"), (CompData.ComponentClass));
			}
		}
	...
	{
		UE_LOG(LPrefab, Warning, TEXT("[ActorSerializer::DeserializeActorRecursiveForConvertToRuntime]Actor Class of index:%d not found!"), (SaveData.ActorClass));
	}
}
...
bool ActorSerializer::LoadCommonPropertyForConvert(FProperty* Property, int itemType, int itemPropertyIndex, const TArray<FLGUIPropertyData>& PropertyData, TArray<FLGUIPropertyDataForBuild>& ResultPropertyData, int cppArrayIndex, bool isInsideCppArray)
{
	...
	if (HaveData)
	{
		...
		else
		{
			...
			{
				//UE_LOG(LPrefab, Error, TEXT("[ActorSerializerNotHandled]FInterfaceProperty:%s"), *(Property->GetFName().ToString()));
			}
			...
			{
				//UE_LOG(LPrefab, Error, TEXT("[ActorSerializerNotHandled]delegateProperty:%s"), *(Property->GetFName().ToString()));
			}
			else if (auto multicastDelegateProperty = CastField<FMulticastDelegateProperty>(Property))//blueprint dynamic delegate
			{
				//UE_LOG(LPrefab, Error, TEXT("[ActorSerializerNotHandled]multicastDelegateProperty:%s"), *(Property->GetFName().ToString()));
			}
			else
			{
				ResultPropertyData.Add(ItemPropertyDataForBuild);
...
void ActorSerializer::ConvertForBuildData(ULPrefab* InPrefab)
{
	...
	{
		UE_LOG(LPrefab, Warning, TEXT("Save binary for build length is 0!"));
		return;
	}
...
void ActorSerializer::GenerateActorIDRecursiveForConvertToRuntime(const FLGUIActorSaveData& SaveData, int32& id)
{
	...
	{
		UE_LOG(LPrefab, Warning, TEXT("[ActorSerializer::GenerateActorIDRecursiveForConvertToRuntime]Actor Class of index:%d not found!"), (SaveData.ActorClass));
	}
}
#endif
