...
AActor* ActorSerializer::DeserializeActorRecursiveForUseInRuntime(USceneComponent* Parent, const FLGUIActorSaveDataForBuild& SaveData, int32& id)
{
	if (auto ActorClass = FindClassFromListByIndex(SaveData.ActorClass, Prefab.Get()))
	{
		...
		{
			UE_LOG(LPrefab, Warning, TEXT("[ActorSerializer::DeserializeActorRecursiveForUseInRuntime]Find class: '%s' at index: %d, but is not a Actor class, use default"), *(ActorClass->GetFName().ToString()), SaveData.ActorClass);
		...
		else
		{
			if (RootCompSaveData.ComponentClass != -1)//have RootComponent data
			{
				if (auto CompClass = FindClassFromListByIndex(RootCompSaveData.ComponentClass, Prefab.Get()))
				{
					...
					{
						UE_LOG(LPrefab, Warning, TEXT("[ActorSerializer::DeserializeActorRecursiveForUseInRuntime]Find class: '%s' at index: %d, but is not a USceneComponent, use default"), *(CompClass->GetFName().ToString()), RootCompSaveData.ComponentClass);
		...
		for (int i = 1; i < ComponentCount; i++)//start from 1, skip RootComponent
		{
			...
			if (auto CompClass = FindClassFromListByIndex(CompData.ComponentClass, Prefab.Get()))
			{
				if (!CompClass->IsChildOf(UActorComponent::StaticClass()))//if not the right class, use default
				{
					...
					UE_LOG(LPrefab, Warning, TEXT("[ActorSerializer::DeserializeActorRecursiveForUseInRuntime]Class:%s is not a UActorComponent, use default"), *(CompClass->GetFName().ToString()));
				}
			...
			else
			{
				...
				UE_LOG(LPrefab, Error, TEXT("%s"), *ErrorMsg); 
#if WITH_EDITOR
				LPrefabUtils::EditorNotification(FText::FromString(ErrorMsg)); 
#endif
			}
		}
