...
AActor* ActorSerializer::DeserializeActorRecursiveForEdit(USceneComponent* Parent, const FLGUIActorSaveData& SaveData, int32& id)
{
	if (auto ActorClass = FindClassFromListByIndex(SaveData.ActorClass, Prefab.Get()))
	{
		...
		{
			UE_LOG(LPrefab, Warning, TEXT("[ActorSerializer::DeserializeActorRecursiveForEdit]Find class: '%s' at index: %d, but is not a Actor class, use default"), *(ActorClass->GetFName().ToString()), SaveData.ActorClass);
		...
		else
		{
			if (RootCompSaveData.ComponentClass != -1)//have RootComponent data
			{
				if (auto CompClass = FindClassFromListByIndex(RootCompSaveData.ComponentClass, Prefab.Get()))
				{
					...
					{
						UE_LOG(LPrefab, Warning, TEXT("[ActorSerializer::DeserializeActorRecursiveForEdit]Find class: '%s' at index: %d, but is not a USceneComponent, use default"), *(CompClass->GetFName().ToString()), RootCompSaveData.ComponentClass);
		...
		for (int i = 1; i < ComponentCount; i++)//start from 1, skip RootComponent
		{
			...
			if (auto CompClass = FindClassFromListByIndex(CompData.ComponentClass, Prefab.Get()))
			{
				if (!CompClass->IsChildOf(UActorComponent::StaticClass()))//if not the right class, use default
				{
					...
					UE_LOG(LPrefab, Error, TEXT("[ActorSerializer::DeserializeActorRecursiveForEdit]Class:%s is not a UActorComponent, use default"), *(CompClass->GetFName().ToString()));
				}
			...
			{
				UE_LOG(LPrefab, Warning, TEXT("[ActorSerializer::DeserializeActorRecursiveForEdit]Component Class of index:%d not found!"), (CompData.ComponentClass));
			}
		}
	...
	{
		UE_LOG(LPrefab, Warning, TEXT("[ActorSerializer::DeserializeActorRecursiveForEdit]Actor Class of index:%d not found!"), (SaveData.ActorClass));
		return nullptr;
	}
}
#endif
