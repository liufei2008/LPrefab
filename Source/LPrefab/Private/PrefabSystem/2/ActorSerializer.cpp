...
ActorSerializer::ActorSerializer(ULPrefab* InPrefab)
{
	Prefab = InPrefab;
}
...
#if WITH_EDITOR
AActor* ActorSerializer::LoadPrefabForEdit(UWorld* InWorld, ULPrefab* InPrefab, USceneComponent* Parent
...
AActor* ActorSerializer::LoadPrefabInEditor(UWorld *InWorld, ULPrefab *InPrefab, USceneComponent *Parent, bool SetRelativeTransformToIdentity)
{
	ActorSerializer serializer(InWorld);
...
void ActorSerializer::RenewActorGuidForDuplicate(ULPrefab* InPrefab)
{
...
AActor* ActorSerializer::DeserializeActor(USceneComponent* Parent, ULPrefab* InPrefab, bool ReplaceTransform, FVector InLocation, FQuat InRotation, FVector InScale)
{
...
#endif
	{
		LPrefabManager = ULPrefabWorldSubsystem::GetInstance(TargetWorld.Get());
	}
	...
	FLGUIActorSaveDataForBuild SaveDataForBuild;
	{
		...
		{
			UE_LOG(LPrefab, Warning, TEXT("Loaded data for build is empty!"));
			return nullptr;
		}
...
#endif
	Prefab = TWeakObjectPtr<ULPrefab>(InPrefab);
	int32 id = 0;
	...
	UE_LOG(LPrefab, Log, TEXT("Use %f seconds to LoadPrefab: %s"), TimeSpan.GetTotalSeconds(), *InPrefab->GetName());
...
void ActorSerializer::LogForBitConvertFail(bool success, FProperty* Property)
{
	if (!success)
	{
		...
		UE_LOG(LPrefab, Error, TEXT("%s"), *msg);
	}
}
...
UObject* ActorSerializer::FindAssetFromListByIndex(int32 Id, ULPrefab* InPrefab)
{
...
FString ActorSerializer::FindStringFromListByIndex(int32 Id, ULPrefab* InPrefab)
{
...
FName ActorSerializer::FindNameFromListByIndex(int32 Id, ULPrefab* InPrefab)
{
...
FText ActorSerializer::FindTextFromListByIndex(int32 Id, ULPrefab* InPrefab)
{
...
UClass* ActorSerializer::FindClassFromListByIndex(int32 Id, ULPrefab* InPrefab)
{
...
FGuid FLGUIActorSaveData::GetActorGuid(FGuid fallbackGuid) const
{
	...
	UE_LOG(LPrefab, Warning, TEXT("[FLGUIActorSaveData::GetActorGuid] Failed to get ActorGuid, maybe this Prefab is created by previous UE version."));
	guid = fallbackGuid;
	return guid;
}
