...
void ActorSerializer::SavePrefab(AActor* RootActor, ULPrefab* InPrefab
	...
	, TArray<AActor*>& OutSerializedActors, TArray<FGuid>& OutSerializedActorsGuid)
{
	...
	{
		UE_LOG(LPrefab, Error, TEXT("[ActorSerializer::SerializeActor]RootActor Or InPrefab is null!"));
		return;
	}
	if (!RootActor->GetWorld())
	{
		UE_LOG(LPrefab, Error, TEXT("[ActorSerializer::SerializeActor]Cannot get World from RootActor!"));
		return;
	}
...
void ActorSerializer::SerializeActor(AActor* RootActor, ULPrefab* InPrefab)
{
	...
	UE_LOG(LPrefab, Log, TEXT("[ActorSerializer::SerializeActor] prefab:%s duration:%s"), *(Prefab->GetPathName()), *((FDateTime::Now() - StartTime).ToString()));
}

FLGUIActorSaveData ActorSerializer::CreateActorSaveData(ULPrefab* InPrefab)
{
	FLPrefabSaveData SaveData;
