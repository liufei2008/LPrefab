	...
	if (auto ActorClass = FindClassFromListByIndex(SaveData.ActorClass, Prefab.Get()))
	{
		...
		for (int i = 0; i < ComponentCount; i++)
		{
			...
			if (auto CompClass = FindClassFromListByIndex(CompData.ComponentClass, Prefab.Get()))
			{
				...
				{
					UE_LOG(LPrefab, Warning, TEXT("[ActorSerializer::DeserializeActorRecursiveForDuplicate]Find class: '%s' at index: %d, but is not a USceneComponent, use default"), *(CompClass->GetFName().ToString()), CompData.ComponentClass);
			...
			{
				UE_LOG(LPrefab, Warning, TEXT("[ActorSerializer::DeserializeActorRecursiveForDuplicate]Component Class of index:%d not found!"), (CompData.ComponentClass));
			}
		}
