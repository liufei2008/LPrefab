...
namespace LPrefabSystem
{
	...
	struct LPrefabSerializerHelper
	{
	...
	public:
		...
		friend FArchive& operator<<(FArchive& Ar, FLGUIPropertyData& PropertyData)
		{
			...
			LPrefabSerializerHelper::SerializeToArchiveByPropertyType(Ar, PropertyData.PropertyType, PropertyData.Data);
	...
	/** This is just a record of old prefab system, will be removed in future release. Use LPrefabSystem3::ActorSerializer3 instead. */
	/*
	...
	*/
	class LPREFAB_API ActorSerializer
	{
	private:
		ActorSerializer(ULPrefab* InPrefab);
	...
	public:
		 ...
		 */
		static AActor* LoadPrefabForEdit(UWorld* InWorld, ULPrefab* InPrefab, USceneComponent* Parent
		 ...
		 */
		static AActor* LoadPrefabInEditor(UWorld* InWorld, ULPrefab* InPrefab, USceneComponent* Parent, bool SetRelativeTransformToIdentity = true);
		static void RenewActorGuidForDuplicate(ULPrefab* InPrefab);

		static FLGUIActorSaveData CreateActorSaveData(ULPrefab* InPrefab);
		static void ConvertForBuildData(ULPrefab* InPrefab);//convert saved data to build data
	...
	private:
		...
		TWeakObjectPtr<ULPrefab> Prefab = nullptr;
...
#endif
		//deserialize actor
		AActor* DeserializeActor(USceneComponent* Parent, ULPrefab* InPrefab, bool ReplaceTransform = false, FVector InLocation = FVector::ZeroVector, FQuat InRotation = FQuat::Identity, FVector InScale = FVector::OneVector);
		...
		//find object by id
		UObject* FindAssetFromListByIndex(int32 Id, ULPrefab* InPrefab);
		FString FindStringFromListByIndex(int32 Id, ULPrefab* InPrefab);
		FName FindNameFromListByIndex(int32 Id, ULPrefab* InPrefab);
		FText FindTextFromListByIndex(int32 Id, ULPrefab* InPrefab);
		UClass* FindClassFromListByIndex(int32 Id, ULPrefab* InPrefab);
