...
#include "LPrefab.h"
#include "LPrefabHelperObject.generated.h"
 ...
 */
UCLASS(ClassGroup = (LPrefab), NotBlueprintType, NotBlueprintable)
class LPREFAB_API ULPrefabHelperObject : public UObject
{
	GENERATED_BODY()

public:	
	ULPrefabHelperObject();
	...
	ULPrefab* GetSubPrefabAsset(AActor* InSubPrefabActor);
	void SavePrefab();
	...
	FLSubPrefabData GetSubPrefabData(AActor* InSubPrefabActor);
	...
	bool RefreshOnSubPrefabDirty(ULPrefab* InSubPrefab, AActor* InSubPrefabRootActor = nullptr);

	void CopyRootObjectParentAnchorData(UObject* InObject, UObject* OriginObject);

	void RevertPrefabPropertyValue(UObject* ContextObject, FProperty* Property, void* ContainerPointerInSrc, void* ContainerPointerInDst, const FLSubPrefabData& SubPrefabData, int RawArrayIndex = 0, bool IsInsideRawArray = false);
	void ApplyPrefabPropertyValue(UObject* ContextObject, FProperty* Property, void* ContainerPointerInSrc, void* ContainerPointerInDst, const FLSubPrefabData& SubPrefabData, int RawArrayIndex = 0, bool IsInsideRawArray = false);
	...
	void MakePrefabAsSubPrefab(ULPrefab* InPrefab, AActor* InActor, const TMap<FGuid, TObjectPtr<UObject>>& InSubMapGuidToObject, const TArray<FLPrefabOverrideParameterData>& InObjectOverrideParameterArray);
	void RemoveSubPrefabByRootActor(AActor* InPrefabRootActor);
	void RemoveSubPrefabByAnyActorOfSubPrefab(AActor* InPrefabActor);
	ULPrefab* GetPrefabAssetBySubPrefabObject(UObject* InObject);
