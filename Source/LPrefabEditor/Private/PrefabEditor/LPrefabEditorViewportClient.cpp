...
#include "LPrefabViewportClickHandlers.h"
#include "LPrefabEditorModule.h"

#define LOCTEXT_NAMESPACE "LPrefabEditorViewportClient"

IMPLEMENT_HIT_PROXY(HLevelSocketProxy, HHitProxy);

FLPrefabEditorViewportClient::FLPrefabEditorViewportClient(FLPrefabEditorScene& InPreviewScene
	, TWeakPtr<FLPrefabEditor> InPrefabEditorPtr
	, const TSharedRef<SLPrefabEditorViewport>& InEditorViewportPtr)
...
FLPrefabEditorViewportClient::~FLPrefabEditorViewportClient()
{
}
...
void FLPrefabEditorViewportClient::Draw(const FSceneView* View, FPrimitiveDrawInterface* PDI)
{
...
void FLPrefabEditorViewportClient::DrawCanvas(FViewport& InViewport, FSceneView& View, FCanvas& Canvas)
{
...
void FLPrefabEditorViewportClient::ReceivedFocus(FViewport* InViewport)
{
...
void FLPrefabEditorViewportClient::LostFocus(FViewport* InViewport)
{
...
void FLPrefabEditorViewportClient::Tick(float DeltaSeconds)
{
...
bool FLPrefabEditorViewportClient::InputKey(const FInputKeyEventArgs& EventArgs)
{
...
void FLPrefabEditorViewportClient::ProcessClick(FSceneView& View, HHitProxy* HitProxy, FKey Key, EInputEvent Event, uint32 HitX, uint32 HitY)
{
	...
	FLPrefabEditorModule::PrefabEditorViewport_MouseClick.ExecuteIfBound(this->GetWorld(), RayOrigin, RayDirection, ClickHitActor);
	if (ClickHitActor != nullptr)
	{
		LPrefabViewportClickHandlers::ClickActor(this, ClickHitActor, Click, true);
		return;
	}
	...
	{
		LPrefabViewportClickHandlers::ClickViewport(this, Click);
		return;
	}
	if (!ModeTools->HandleClick(this, HitProxy, Click))
	{
		...
		if (HitProxy == NULL)
		{
			LPrefabViewportClickHandlers::ClickBackdrop(this, Click);
		}
		...
		else if (HitElement && LPrefabViewportClickHandlers::ClickElement(this, HitElement, Click))
		{
		...
		else if (HitProxy->IsA(HActor::StaticGetType()))
		{
			...
			if (ConsideredActor) // It is possible to be clicking something during level transition if you spam click, and it might not be valid by this point
			{
				...
				if (bSelectComponent)
				{
					bComponentSelected = LPrefabViewportClickHandlers::ClickComponent(this, ActorHitProxy, Click);
				}

				if (!bComponentSelected)
				{
					LPrefabViewportClickHandlers::ClickActor(this, ConsideredActor, Click, true);
				}
		...
		{
			LPrefabViewportClickHandlers::ClickActor(this, ((HInstancedStaticMeshInstance*)HitProxy)->Component->GetOwner(), Click, true);
		}
		...
		//	LPrefabViewportClickHandlers::ClickBrushVertex(this, ((HBSPBrushVert*)HitProxy)->Brush.Get(), &Vertex, Click);
		//}
		else if (HitProxy->IsA(HStaticMeshVert::StaticGetType()))
		{
			LPrefabViewportClickHandlers::ClickStaticMeshVertex(this, ((HStaticMeshVert*)HitProxy)->Actor, ((HStaticMeshVert*)HitProxy)->Vertex, Click);
		}
		...
		else if (HitProxy->IsA(HModel::StaticGetType()))
		{
			...
			{
				LPrefabViewportClickHandlers::ClickSurface(this, ModelHit->GetModel(), SurfaceIndex, Click);
			}
		}
		else if (HitProxy->IsA(HLevelSocketProxy::StaticGetType()))
		{
			LPrefabViewportClickHandlers::ClickLevelSocket(this, HitProxy, Click);
		}
	}
}

bool FLPrefabEditorViewportClient::InputWidgetDelta(FViewport* InViewport, EAxisList::Type InCurrentAxis, FVector& Drag, FRotator& Rot, FVector& Scale)
{
...
UE::Widget::EWidgetMode FLPrefabEditorViewportClient::GetWidgetMode() const
{
...
FVector FLPrefabEditorViewportClient::GetWidgetLocation() const
{
...
FMatrix FLPrefabEditorViewportClient::GetWidgetCoordSystem() const
{
...
int32 FLPrefabEditorViewportClient::GetCameraSpeedSetting() const
{
	return GetDefault<UEditorPerProjectUserSettings>()->SCSViewportCameraSpeed;
}
void FLPrefabEditorViewportClient::SetCameraSpeedSetting(int32 SpeedSetting)
{
...
EAxisList::Type FLPrefabEditorViewportClient::GetHorizAxis() const
{
...
EAxisList::Type FLPrefabEditorViewportClient::GetVertAxis() const
{
	switch (GetViewportType())
	{
	case LVT_OrthoXY:
...
void FLPrefabEditorViewportClient::NudgeSelectedObjects(const struct FInputEventState& InputState)
{
...
void FLPrefabEditorViewportClient::ApplyDeltaToActors(const FVector& InDrag, const FRotator& InRot, const FVector& InScale)
{
	ApplyDeltaToSelectedElements(FTransform(InRot, InDrag, InScale));
}

void FLPrefabEditorViewportClient::ApplyDeltaToActor(AActor* InActor, const FVector& InDeltaDrag, const FRotator& InDeltaRot, const FVector& InDeltaScale)
{
...
void FLPrefabEditorViewportClient::ApplyDeltaToComponent(USceneComponent* InComponent, const FVector& InDeltaDrag, const FRotator& InDeltaRot, const FVector& InDeltaScale)
{
...
void FLPrefabEditorViewportClient::ApplyDeltaToSelectedElements(const FTransform& InDeltaTransform)
{
...
void FLPrefabEditorViewportClient::ApplyDeltaToElement(const FTypedElementHandle& InElementHandle, const FTransform& InDeltaTransform)
{
	FInputDeviceState InputState;
	InputState.SetModifierKeyStates(IsShiftPressed(), IsAltPressed(), IsCtrlPressed(), IsCmdPressed());
...
FTypedElementListConstRef FLPrefabEditorViewportClient::GetElementsToManipulate(const bool bForceRefresh)
{
	CacheElementsToManipulate(bForceRefresh);
	return CachedElementsToManipulate;
}

void FLPrefabEditorViewportClient::CacheElementsToManipulate(const bool bForceRefresh)
{
	if (bForceRefresh)
	{
...
void FLPrefabEditorViewportClient::ResetElementsToManipulate(const bool bClearList)
{
	if (bClearList)
	{
...
void FLPrefabEditorViewportClient::ResetElementsToManipulateFromSelectionChange(const UTypedElementSelectionSet* InSelectionSet)
{
...
void FLPrefabEditorViewportClient::ResetElementsToManipulateFromProcessingDeferredElementsToDestroy()
{
	if (!bHasCachedElementsToManipulate)
	{
...
const UTypedElementSelectionSet* FLPrefabEditorViewportClient::GetSelectionSet() const
{
	return GEditor->GetSelectedActors()->GetElementSelectionSet();
}

UTypedElementSelectionSet* FLPrefabEditorViewportClient::GetMutableSelectionSet() const
{
	return GEditor->GetSelectedActors()->GetElementSelectionSet();
}


void FLPrefabEditorViewportClient::TickWorld(float DeltaSeconds)
{
	GetWorld()->Tick(LEVELTICK_All, DeltaSeconds);
}

bool FLPrefabEditorViewportClient::FocusViewportToTargets()
{
...
UWorld* FLPrefabEditorViewportClient::GetWorld()const
{
	return PrefabScene->GetWorld();
}
void FLPrefabEditorViewportClient::AddReferencedObjects(FReferenceCollector& Collector)
{
...
void FLPrefabEditorViewportClient::DrawPreviewLightVisualization(const FSceneView* View, FPrimitiveDrawInterface* PDI)
{
...
FLinearColor FLPrefabEditorViewportClient::GetBackgroundColor() const
{
...
bool FLPrefabEditorViewportClient::Internal_InputAxis(FViewport* InViewport, FInputDeviceId DeviceID, FKey Key, float Delta, float DeltaTime, int32 NumSamples, bool bGamepad)
{
	if (bDisableInput)
	{
		return true;
...
ULPrefab* FLPrefabEditorViewportClient::GetPrefabBeingEdited()const
{
...
void FLPrefabEditorViewportClient::GetSelectedActorsAndComponentsForMove(TArray<AActor*>& OutActorsToMove, TArray<USceneComponent*>& OutComponentsToMove) const
{
...
bool FLPrefabEditorViewportClient::CanMoveActorInViewport(const AActor* InActor) const
{
	if (!GEditor || !InActor)
	{
		return false;
...
void FLPrefabEditorViewportClient::CapturedMouseMove(FViewport* InViewport, int32 InMouseX, int32 InMouseY)
{
	...
	{
		FLPrefabEditorModule::PrefabEditorViewport_MouseMove.ExecuteIfBound(this->GetWorld());
	}
	PrevMouseX = InMouseX;
	PrevMouseY = InMouseY;
}

void FLPrefabEditorViewportClient::MouseEnter(FViewport* InViewport, int32 x, int32 y)
{
	FEditorViewportClient::MouseEnter(InViewport, x, y);
}
void FLPrefabEditorViewportClient::MouseMove(FViewport* InViewport, int32 x, int32 y)
{
	FEditorViewportClient::MouseMove(InViewport, x, y);
}
void FLPrefabEditorViewportClient::MouseLeave(FViewport* InViewport)
{
	FEditorViewportClient::MouseLeave(InViewport);
}

void FLPrefabEditorViewportClient::TrackingStarted(const struct FInputEventState& InInputState, bool bIsDraggingWidget, bool bNudge)
{
...
void FLPrefabEditorViewportClient::TrackingStopped()
{
...
void FLPrefabEditorViewportClient::AbortTracking()
{
...
bool FLPrefabEditorViewportClient::HaveSelectedObjectsBeenChanged() const
{
