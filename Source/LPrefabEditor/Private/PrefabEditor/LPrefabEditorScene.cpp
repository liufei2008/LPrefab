...
#define LOCTEXT_NAMESPACE "LPrefabEditorScene"

const FString FLPrefabEditorScene::RootAgentActorName = TEXT("[temporary_RootAgent]");

FLPrefabEditorScene::FLPrefabEditorScene(ConstructionValues CVS) :FPrefabScene(CVS)
{
...
USceneComponent* FLPrefabEditorScene::GetParentComponentForPrefab(ULPrefab* InPrefab)
{
	...
	if (InPrefab->GetIsPrefabVariant())
	{
		...
		while (RootSubPrefab->GetIsPrefabVariant())
		{
			...
			RootSubPrefab = Cast<ULPrefab>(RootSubPrefab->ReferenceAssetList[0]);
			if (!RootSubPrefab)
			{
				return nullptr;
			}
	...
	{
		if (!FLPrefabEditorModule::PrefabEditor_CreateRootAgent.ExecuteIfBound(this->GetWorld(), Prefab->ReferenceClassList[0], InPrefab, RootAgentActor))
		{
...
bool FLPrefabEditorScene::IsWorldDefaultActor(AActor* InActor)const
{
